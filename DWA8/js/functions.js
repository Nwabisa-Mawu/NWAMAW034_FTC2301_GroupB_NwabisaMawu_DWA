...
export const appendBooks = (books) => {
  ...
  data.home.showMoreBtn.innerHTML = `Show more <span class = "list__remaining">(${updateBooksLeft() - BOOKS_PER_PAGE})</span>`;
};
...
export const showMoreAction = (event) => {
  ...
  if (booksLeft > 36) {
    ...
    data.home.showMoreBtn.innerHTML = `Show more <span class="list__remaining">(${booksLeft - BOOKS_PER_PAGE})</span>`;
  } else {
    books.slice(0, booksLeft).forEach((book) => {
      createBookButtons(book);
    });

    data.home.main.appendChild(FRAGMENT);
    data.home.showMoreBtn.innerHTML = 'Show more <span class="list__remaining">(0)</span>';
    data.home.showMoreBtn.disabled = true;
  }
